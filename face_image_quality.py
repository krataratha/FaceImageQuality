...
class SER_FIQ:
    ...
    def get_scores_batch(self, aligned_imgs: list, 
                              T: int = 100, 
                              alpha: float = 130.0, 
                              r: float = 0.88):
        """
        Calculates SER-FIQ scores for a list of aligned images efficiently by 
        stacking their forward passes together into a single network invocation.

        Parameters
        ----------
        aligned_imgs : list of np.ndarray
            A list containing aligned face images.
        T : int, optional
            Amount of forward passes to use per image.
        alpha : float, optional
            Stretching factor.
        r : float, optional
            Score displacement.
        ...
        scores : list of float
            List of calculated SER-FIQ scores matching the input sequence.
        """
        if not aligned_imgs:
            return []

        processed_imgs = [
            np.transpose(img, (2, 0, 1)) if img.shape[0] != 3 else img 
            for img in aligned_imgs
        ]
        
        # Shape: (Num_Images * T, 3, H, W)
        batch_blob = np.concatenate([np.repeat(np.expand_dims(img, axis=0), T, axis=0) for img in processed_imgs], axis=0)
        gpu_batch = mx.nd.array(batch_blob, ctx=self.device)
        
        all_features = self.insightface(gpu_batch).asnumpy()
        normalized_features = normalize(all_features, axis=1)
        
        scores = []
        triu_idx = np.triu_indices(T, k=1)
        
        for i in range(len(aligned_imgs)):
            # Extract features belonging to the i-th image block
            img_features = normalized_features[i * T : (i + 1) * T]
            eucl_dist = euclidean_distances(img_features, img_features)[triu_idx]
            
            score = 2 * (1 / (1 + np.exp(np.mean(eucl_dist))))
            final_score = 1 / (1 + np.exp(-(alpha * (score - r))))
